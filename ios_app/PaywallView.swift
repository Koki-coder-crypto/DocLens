...
struct PaywallView: View {
    ...
    @State private var selectedPlan: String = "com.kokicoder.doclens.pro.annual"
    ...
    private var heroGlow: some View {
        ZStack {
            ...
            Ellipse().fill(Color(hex: "A78BFA").opacity(0.07)).frame(width: 220, height: 180)
    ...
    private var heroSection: some View {
        VStack(spacing: 18) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Unlimited AI Document Scanning")
    ...
    private var planPicker: some View {
        VStack(spacing: 10) {
            if let p = store.proAnnual {
                DocPlanCard(title: "Annual", displayPrice: p.displayPrice, period: "/ year",
            ...
            if let p = store.proMonthly {
                DocPlanCard(title: "Monthly", displayPrice: p.displayPrice, period: "/ month",
            ...
            Button { Task { await startPurchase() } } label: {
                ZStack {
                    if isPurchasing { ProgressView().tint(.white) }
                        ...
                        HStack(spacing: 8) {
                            Text(selectedPlan.contains("annual") ? "Start Free Trial" : "Get DocLens Pro")
    ...
    private func startPurchase() async {
        ...
        do {
            try await store.purchase(product); Haptics.notification(.success); dismiss()
        } catch {
            Haptics.notification(.error); errorMessage = "Purchase failed. Please try again."
        }
        isPurchasing = false
    }
}

struct DocPlanCard: View {
